...
// Forwards the signed-in user's access token (SaveTokens above) so SurveyApp.Api
// sees the same bearer token Advantage.Identity issued — no separate service auth.
builder.Services.AddHttpClient("SurveyAppApi", client =>
{
    client.BaseAddress = new Uri(builder.Configuration["SurveyAppApi:BaseUrl"]!);
});
